...
export interface ScheduledTaskView {
	...
	lastError?: string;
	runs: Array<{ at: string; status: "success" | "failed" | "skipped"; detail?: string }>;
}
...
export function ScheduledTasksPanel({ tasks, onSave, onRemove, onRunNow }: ScheduledTasksPanelProps) {
		...
		<div className="scheduled-tasks-panel">
			<header>
				<div>
					...
					<p>Runs in the Electron background lifecycle.</p>
				</div>
				...
				{tasks.length === 0 ? (
					...
					tasks.map(task => (
						<article key={task.id}>
							...
							{task.runs.length ? (
								<small>
									Last diagnostic: {task.runs.at(-1)?.at} · {task.runs.at(-1)?.status}
								</small>
							) : null}
						</article>
					))
