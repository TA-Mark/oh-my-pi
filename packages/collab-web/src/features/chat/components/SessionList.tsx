...
export function SessionList({ sessions, activeId, loading, onActivate, onDelete, onNew }: Props): ReactNode {
	return (
		<div>
			...
			{loading && sessions.length === 0 && <div style={{ fontSize: 12, color: "var(--fg-faint)" }}>Loading…</div>}

			{!loading && sessions.length === 0 && (
				<div style={{ fontSize: 12, color: "var(--fg-faint)" }}>No sessions yet. Start one below.</div>
			)}
			...
			<div className="mc-session-list">
				{sessions.map(s => (
					<div
						...
						onKeyDown={e => {
							if (e.key === "Enter") onActivate(s.id, s.link);
						}}
					>
			...
			<button type="button" className="mc-new-session-btn" onClick={onNew} disabled={loading}>
				+ New Session
			</button>
		</div>
	);
