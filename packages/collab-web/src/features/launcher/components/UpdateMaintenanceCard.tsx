...
import type { UpdateChannel, UpdateInfo } from "../types/launcher";
		...
		<div className="lnc-card">
			...
			{updateInfo?.available && (
				<div className="lnc-update-banner">
					<div className="lnc-update-text">
						Update available: <span className="lnc-update-version">{updateInfo.latestVersion}</span> (current:{" "}
					...
					>
						{updating ? (
							<>
								<span className="lnc-spinner" /> Updating…
							</>
						) : (
							"↑ Update"
						)}
			...
			{updateInfo && !updateInfo.available && (
				<div style={{ fontSize: 12, color: "var(--fg-muted)" }}>
					...
					<span style={{ color: "var(--fg-faint)", marginLeft: 6 }}>({updateInfo.channel})</span>
				</div>
			)}

			{/* Maintenance actions */}
			<div className="lnc-maintenance-grid">
				<button className="lnc-btn" onClick={onCheckUpdate} disabled={updateChecking || updating}>
					{updateChecking ? (
						<>
							<span className="lnc-spinner" /> Checking…
						</>
					) : (
						"⟳ Check Update"
					)}
