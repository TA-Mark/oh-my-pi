	...
	return (
		<div className="lnc-card">
			...
			<div className="lnc-control-grid">
				{/* Start */}
				<button className="lnc-btn lnc-btn-primary" onClick={onStart} disabled={isRunning || isTransition}>
					{phase === "starting" ? (
						<>
							<span className="lnc-spinner" /> Starting…
						</>
					) : (
						"▶ Start"
					)}
				...
				{/* Stop */}
				<button className="lnc-btn lnc-btn-danger" onClick={onStop} disabled={isStopped || isTransition || isError}>
					{phase === "stopping" ? (
						<>
							<span className="lnc-spinner" /> Stopping…
						</>
					) : (
						"■ Stop"
					)}
				...
				{/* Restart */}
				<button className="lnc-btn" onClick={onRestart} disabled={isStopped || isTransition || isError}>
					↺ Restart
				</button>
			...
			{isTransition && (
				<div style={{ fontSize: 11, color: "var(--fg-faint)" }}>Controls locked while service transitions…</div>
			)}
		</div>
	);
}
