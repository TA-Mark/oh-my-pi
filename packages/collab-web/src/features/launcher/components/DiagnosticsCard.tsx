	...
	return (
		<div className="lnc-card lnc-card-span">
			<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
				...
				<button className="lnc-btn" onClick={onRunDiag} disabled={running}>
					{running ? (
						<>
							<span className="lnc-spinner" /> Running…
						</>
					) : (
						"⚕ Run Diagnostics"
					)}
			...
			{checks.length === 0 && !running && (
				<div style={{ fontSize: 12, color: "var(--fg-faint)" }}>Run diagnostics to check service health.</div>
			)}
